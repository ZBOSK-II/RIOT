...
char *thread_stack_init(thread_task_func_t task_func,
                        ...
                        int stack_size)
{
    printf("[BOOT] thread_stack_init: func=%p stack=%p size=%d\r\n",
           (void *)task_func, stack_start, stack_size);
    struct context_switch_frame *sf;
    uint64_t *stk_top;

    /* calculate the top of the stack */
    stk_top = (uint64_t *)((uintptr_t)stack_start + stack_size);
    ...
    *stk_top = STACK_MARKER;
    ...
    stk_top = (uint64_t *)(((uintptr_t)stk_top) & ~((uintptr_t)0xf));

    /* reserve space for the stack frame. */
    stk_top = (uint64_t *)((uintptr_t)stk_top - sizeof(*sf));
...
void thread_print_stack(void)
{
     ...
     * -Wcast-align */
    uint64_t *sp = (uint64_t *)(uintptr_t)active_thread->sp;
    ...
    printf("stack start: 0x%016" PRIxPTR "\n", (uintptr_t)(active_thread->stack_start));
    printf("stack end  : 0x%016" PRIxPTR "\n",
    ...
    do {
        printf("  0x%016" PRIxPTR ":   0x%016" PRIx64 "\n", (uintptr_t)sp, *sp);
        sp++;
        count++;
...
void heap_stats(void)
{
    ...
    printf("heap: %ld (used %lu, free %ld) [bytes]\n",
           heap_size, (unsigned long)minfo.uordblks, heap_size - minfo.uordblks);
}
