...
typedef struct {
    ...
    /**
     * @brief   Pointer to the TX descriptor ring (set in init, points to
     *          a static 1024-byte-aligned file-scope array in greth.c)
     */
    greth_desc_t *tx_desc;

    /**
     * @brief   Pointer to the RX descriptor ring (same as tx_desc)
     */
    greth_desc_t *rx_desc;

    /**
     * @brief   Pointer to the flat TX frame buffer (set in init)
     */
    uint8_t *tx_buf;
