 ...
 * (-2) avoids wasting 200 k iterations.
 * Returns 16-bit value, or: -1 NVALID, -2 bus inaccessible, -3 stuck. */
...
static void _phy_configure_mac(greth_t *dev)
{
    ...
    if (phy_ctrl == -2 || phy_stat == -2) {
        ...
        printf("[greth] PHY MDIO unexpected (ctrl=%d stat=%d), defaulting 100FD\n",
               phy_ctrl, phy_stat);
        full_duplex = true;
        speed_100   = true;
    }
